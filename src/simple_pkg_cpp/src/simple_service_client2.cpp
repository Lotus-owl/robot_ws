...
public:
    ...
    void send_request()
    {
        while (!_client->wait_for_service(1s)) //연결
        {
            if (!rclcpp::ok())
            {
                RCLCPP_ERROR(get_logger(), "Reqeust time out");
            }
            RCLCPP_INFO(get_logger(), "Connecting again, please wait...");
        }
        //변수 선언
        ...
        auto result_future = _client->async_send_request(
            request, std::bind(&SimpleServiceClient::response_callback, this, std::placeholders::_1));  
    }
